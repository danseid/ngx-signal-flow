...
const coverageColor = percentage => {
  const thresholds = [[95, 'brightgreen'], [90, 'green'], [80, 'yellowgreen'], [70, 'yellow'], [60, 'orange']];
...
const badges = {
  ...
  'core-bundle-size.json': badge('createCoreStore gzip', kilobytes(performance.bundle.createCoreStore.gzipBytes), 'blue')
};

const outputDirectory = resolve(workspaceDirectory, options.out);
await mkdir(outputDirectory, {recursive: true});
await Promise.all(Object.entries(badges).map(([fileName, content]) =>

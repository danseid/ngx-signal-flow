...
describe('store-level reducers and effects over several sources', () => {
  ...
  class FlightSearchComponent {
    ...
    readonly route = this.store.compute('origin', 'destination', 'date', (origin, destination, date) =>
      origin ? `${origin}-${destination} ${date}` : 'incomplete',
    );
    ...
    private readonly flightsEffect = this.store.effect(
      this.origin,
      this.destination,
      this.date,
      (origin, destination, date) => this.http.get<Flight[]>('/api/flights', {params: {origin, destination, date}}),
    );
    readonly flightsLoading = this.flightsEffect.loading;

    constructor() {
      const routeReducer = this.store.reduce(
        this.origin,
        this.destination,
        this.date,
        (draft, origin, destination, date) => {
          draft.origin = origin;
      ...
      this.store
        .effect(this.origin, (origin) => this.http.get<string>(`/api/airports/${origin}`))
        .reduce((draft, name) => {
          draft.airportName = name;
        });
      this.store
        .effect(this.origin, this.destination, (origin, destination) =>

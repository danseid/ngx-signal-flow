...
describe('plain store integration', () => {
  ...
  describe('selectors and computed values', () => {
    ...
    class InvoiceComponent {
      private readonly store = createStore<Invoice>({
        net: 100,
        taxRate: 0.2,
        discount: 10,
        shipping: 5,
      ...
      readonly subtotal = this.store.compute(
        'net',
        'discount',
        'shipping',
        (net, discount, shipping) => net - discount + shipping,
      );
      readonly withTax = this.store.compute(
        'net',
        'discount',
        'shipping',
        'taxRate',
      ...
      readonly total = this.store.compute(
        'net',
        'discount',
        'shipping',
        'taxRate',
    ...
    it('renders select and compute with one to five keys and updates them after a reducer', async () => {
      ...
      expect(JSON.parse(textOf(fixture, '.snapshot')!)).toEqual({
        net: 200,
        taxRate: 0.2,
        discount: 10,
        shipping: 5,

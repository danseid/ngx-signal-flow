...
describe('source integration', () => {
  ...
  describe('connecting Angular signals', () => {
    @Component({
      ...
      template: `<p class="name">{{ name() }}</p>
        <p class="nickname">{{ nickname() ?? 'none' }}</p>`,
    })
  ...
  describe('destroying a source', () => {
    @Component({
      changeDetection: ChangeDetectionStrategy.OnPush,
      template: `<p class="query">{{ query() }}</p>
        <p class="results">{{ results().join(',') }}</p>`,
    })
    class SearchComponent {
      ...
      constructor() {
        ...
        this.search
          .effect((query) => this.http.get<string[]>('/api/search', {params: {q: query}}))

...
describe('core store integration', () => {
  ...
  @Injectable()
  class OrderStore {
    readonly store = createCoreStore<Order>({
      items: ['apples', 'bread'],
      filter: '',
      discount: 0,
      shipping: 4,
    ...
    readonly visibleItems = this.store.compute('items', 'filter', (items, filter) =>
      items.filter((item) => item.includes(filter)),
    );
    readonly count = this.store.compute('items', (items) => items.length);
    readonly price = this.store.compute(
      'items',
      'discount',
      'shipping',
      (items, discount, shipping) => items.length * 10 - discount + shipping,
    );
    readonly priceLabel = this.store.compute(
      'items',
      'discount',
      'shipping',
      'currency',
    ...
    readonly summary = this.store.compute(
      ...
      'shipping',
      'currency',
      (items, filter, discount, shipping, currency) =>
        `${items.length} items, filter "${filter}", ${items.length * 10 - discount + shipping} ${currency}`,
    );
    countRuns = 0;
